...
function getColorClass(letter: string) {
  ...
  return colors[letter.toLowerCase()] || "bg-gradient-to-br from-gray-500 to-gray-600";
}

export function EmployeeFilter() {
  const { employees, selectedDepartment, setEmployeeChecked } = useFilterStore();
          ...
          <div key={item.id} className="flex items-center justify-between space-x-2">
            <div className="relative flex items-center gap-2">
              <input
                ...
                onChange={(e) => setEmployeeChecked(item.id, e.target.checked)}
